(function (global, factory) {
  ...
  function updateNOverdueOptions() {
    ...
    ['ordinalChart', 'linearChart', 'listing'].forEach(function (display) {
      console.log(_this[display].filters); //this[display].filters.find(filter => filter.col === 'nOverdue').value = 'All';
    });
  }
  ...
  function styles() {
    ...
    /****---------------------------------------------------------------------------------\
      Charts
    \---------------------------------------------------------------------------------****/
    '.pvl-charts {' + '    width: 100%;' + '    display: inline-block;' + '}', '.pvl-chart {' + '    display: inline-block;' + '    width: 100%;' + '}', '.pvl-charts .pvl-chart--ordinal {' + '    width: 49.5%;' + '    float: left;' + '}', '.pvl-charts .pvl-chart--linear {' + '    width: 49.5%;' + '    float: right;' + '}', '.pvl-chart .axis-title--top {' + '    font-size: 16px;' + '    font-weight: bold;' + '}' + '.pvl-chart--full .pvl-chart-button {' + '    display: none;' + '}', '.pvl-chart .pvl-chart-button {' + '    font-size: 30px;' + '    cursor: pointer;' + '    fill: black;' + '}' + '.pvl-chart .pvl-chart-button:hover {' + '    fill: blue;' + '    stroke: blue;' + '}' + '.pvl-chart .pvl-chart-button--minimize {' + '}' + '.pvl-chart .pvl-chart-button--split {' + '    font-size: 24px;' + '}' + '.pvl-chart .pvl-chart-button--split:hover {' + '}' + '.pvl-chart .pvl-chart-button--maximize {' + '}' + '.pvl-unscheduled-legend-item,' + '.pvl-unscheduled-annotation {' + '    font-size: 14px;' + '    font-family: courier;' + '}',
    /****---------------------------------------------------------------------------------\
      Listing
    \---------------------------------------------------------------------------------****/
  ...
  function addAnnotationLegend() {
    ...
    if (this.pvl.data.sets.unscheduledVisits.length) this.topXAxis.container.selectAll('.pvl-unscheduled-legend-item').remove();
    this.pvl.data.sets.unscheduledVisits.forEach(function (visit, i) {
      _this.topXAxis.container.append('text').datum(visit).classed('pvl-unscheduled-legend-item', true).attr({
        transform: "translate(-".concat(_this.margin.left - 15, ",").concat(-_this.margin.top + 16 * (i + 1) + 3, ")")
