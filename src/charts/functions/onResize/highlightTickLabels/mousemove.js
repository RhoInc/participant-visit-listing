...
export default function mousemove(mouse) {
    ...
    if (this.config.x.type === 'ordinal') {
        const x_coord = this.x_coords
            .find(x_coord => x_coord.coord <= x && x < (x_coord.coord + this.x.rangeBand()));
        if (x_coord) {
            this.wrap
    ...
    //y
    const y = coords[1];
    const y_coord = this.y_coords
        .find(y_coord => y_coord.coord <= y && y < (y_coord.coord + this.y.rangeBand()));
    if (y_coord) {
        this.wrap
