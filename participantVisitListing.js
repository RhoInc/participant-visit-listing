...
function layout() {
    ...
    this.containers.tabs = this.containers.tabContainer.selectAll('div').data(['Charts', 'Listing']).enter().append('div').attr('class', function (d) {
        return 'pvl-tab pvl-tab--' + d.toLowerCase() + ' ' + (d === 'Charts' ? 'pvl-tab--active' : '');
    }).text(function (d) {
        return d;
    });
...
function styles() {
  ...
  '.pvl-charts {' + '    width: 100%;' + '    display: inline-block;' + '}', '.pvl-chart {' + '    display: inline-block;' + '}', '.pvl-chart--ordinal {' + '    width: 33%;' + '    float: left;' + '}', '.pvl-chart--linear {' + '    width: 66%;' + '    float: right;' + '}', '.pvl-chart .axis-title--top {' + '    font-size: 16px;' + '    font-weight: bold;' + '}' +
...
function update$1() {
    ...
    }).selectAll('select').on('change', function (d) {
