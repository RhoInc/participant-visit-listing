(function (global, factory) {
  ...
  function highlightVisit() {
    ...
    this.marks.filter(function (mark) {
      ...
      mark.groups.on('click', function (d) {
        _this.svg.selectAll('.pvl-highlighted-visit').remove();

        console.log(d);
        _this.highlightedVisit = d.values.raw[0][_this.pvl.settings.visit_col];

        _this.svg.selectAll('.point circle').attr({
          'stroke-opacity': .5,
          'fill-opacity': .5
        });

        _this.svg.selectAll('.point').filter(function (di) {
          return di.values.raw[0][_this.pvl.settings.visit_col] === _this.highlightedVisit;
        }).each(function (di) {
          var point = d3.select(this);
          point.insert('line', ':first-child').classed('pvl-highlighted-visit', true).attr({
