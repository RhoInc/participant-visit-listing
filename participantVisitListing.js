(function (global, factory) {
  ...
  function syncListingSettings() {
    ...
    if (['tabbed', 'side-by-side'].indexOf(settings.chart_layout) < 0) {
      console.warn("[ chart_layout ] must be \"tabbed\" or \"side-by-side\", not \"".concat(settings.chart_layout, "\". Defaulting to \"tabbed\"."));
    ...
    if (settings.chart_layout === 'tabbed') {
      if (['Visit Chart', 'Study Day Chart', 'Listing'].indexOf(settings.active_tab) < 0) {
        console.warn("[ active_tab ] must be \"Visit Chart\", \"Study Day Chart\", or \"Listing\", not \"".concat(settings.active_tab, "\". Defaulting to \"Visit Chart\"."));
        settings.active_tab = 'Visit Chart';
      }
    } else if (settings.chart_layout === 'side-by-side') {
      if (['Charts', 'Listing'].indexOf(settings.active_tab) < 0) {
        console.warn("[ active_tab ] must be \"Charts\" or \"Listing\", not \"".concat(settings.active_tab, "\". Defaulting to \"Charts\"."));
  ...
  function syncOrdinalChartSettings() {
    ...
    circles.tooltip = "[".concat(listingSettings.id_col, "] - [").concat(listingSettings.visit_col, "] ([").concat(listingSettings.visit_date_col, "]: Day [").concat(listingSettings.visit_day_col, "]): [").concat(listingSettings.visit_status_col, "]");
  ...
  function loading(event, callback) {
    ...
    var loading = setInterval(function () {
      ...
      if (loadingIndicated) {
        ...
        if (!_this.test) console.log("".concat(event, " took ").concat(t1 - t0, " milliseconds."));
      }
    });
  }
  ...
  function updateNOverdueOptions() {
    ...
    var nOverdue = ['All'].concat(_toConsumableArray(d3.set(this.data.analysis.map(function (d) {
      return d.nOverdue;
    })).values())); //  Upate options in # of Overdue Visits dropdown.

    var nOverdueOptions = this.controls.wrap.selectAll('.control-group select').filter(function (d) {
      return d.value_col === 'nOverdue';
    }).selectAll('option').data(nOverdue);
    nOverdueOptions.enter().append('option').text(function (d) {
      return d;
    });
  ...
  function filterData(d, select) {
    ...
    if (d) {
      ...
      if (select) filter.value = select.multiple ? d3.select(select).selectAll('option:checked').data() : select.value;
  ...
  function defineDefaultSet(dataMapping) {
    ...
    if (variable !== undefined) {
      this.data.sets[dataMapping] = d3.set(this.data.filtered.map(function (d) {
        return d[variable];
      })).values().sort(); //  Sort set numerically if possible.

      if (this.data.sets[dataMapping].every(function (value) {
      ...
      this.data.sets[variable] = d3.set(this.data.filtered.map(function (d) {
        return d[variable];
      })).values().sort(); //  Sort set numerically if possible.
  ...
  function defineVisitSet() {
    ...
    this.data.sets.visit_col = d3.set(this.data.analysis.map(function (d) {
      return "".concat(d[_this.settings.visit_order_col], ":|:").concat(d[_this.settings.visit_col], ":|:").concat(d[_this.settings.visit_abbreviation_col]);
    ...
    this.data.sets.scheduledVisits = this.data.sets.visit_col.filter(function (visit) {
      return _this.settings.visit_exclusion_regex ? !_this.settings.visit_exclusion_regex.test(visit.name) : true;
    }).map(function (visit) {
      return visit.name;
    }); //  unscheduled visit set

    this.data.sets.unscheduledVisits = d3.set(this.data.sets.visit_col.filter(function (visit) {
      return _this.settings.visit_exclusion_regex ? _this.settings.visit_exclusion_regex.test(visit.name) : false;
    }).sort(function (a, b) {
  ...
  function transposeData() {
    ...
    try {
      var _loop = function _loop() {
        ...
        var datum = (_datum = {}, _defineProperty(_datum, _this.settings.site_col, id_data[0][_this.settings.site_col]), _defineProperty(_datum, "Site", id_data[0][_this.settings.site_col]), _defineProperty(_datum, _this.settings.id_col, id), _defineProperty(_datum, "ID", id), _defineProperty(_datum, _this.settings.id_status_col, id_data[0][_this.settings.id_status_col]), _defineProperty(_datum, "Status", id_data[0][_this.settings.id_status_col]), _defineProperty(_datum, "nOverdue", id_data[0].nOverdue), _datum);
        ...
        try {
          var _loop2 = function _loop2() {
            ...
            datum[visit.name] = visit_datum ? visit_datum[_this.settings.visit_text_col] : '';
            datum["".concat(visit.name, "-date")] = visit_datum ? visit_datum[_this.settings.visit_date_col] : '';
            datum["".concat(visit.name, "-status")] = visit_datum ? visit_datum[_this.settings.visit_status_col] : '';
            datum["".concat(visit.name, "-color")] = visit_datum ? visit_datum[_this.settings.visit_status_color_col] : '';
          ...
          for (var _iterator2 = _this.data.sets.visit_col[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
            _loop2();
          }
        } catch (err) {
      ...
      for (var _iterator = this.data.sets.id_col[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
        _loop();
      }
    } catch (err) {
  ...
  function update$1() {
    ...
    filters.on('change', function (d) {
      ...
      loading.call(context, "".concat(d.label, " filter change"), function () {
        ...
        context.controls.reset.button.property('disabled', context.data.filters.every(function (filter) {
          return filter.value === 'All' || Array.isArray(filter.value) && filter.value.join('') === filter.set.join('');
  ...
  function addResetButton() {
    ...
    this.controls.reset = {
      button: this.controls.wrap.insert('button', ':first-child').classed('pvl-reset-button', true).property('disabled', this.data.filters.every(function (filter) {
        return filter.value === 'All' || Array.isArray(filter.value) && filter.value.join('') === filter.set.join('');
      })).text('Reset Filters'),
      action: function action() {
        ...
        loading.call(this, 'Reset controls', function () {
          ...
          if (_this.ordinalChart.initialized) _this.ordinalChart.raw_data = _this.data.raw;
          ...
          _this.data.filters.forEach(function (filter) {
            ...
            _this.displays.forEach(function (display) {
              ...
              if (displayFilter) displayFilter.val = displayFilter.all ? 'All' : displayFilter.choices.slice();
            });
          ...
          _this.displays.filter(function (display) {
      ...
      loading.call(context, "this.containers.tabs.".concat(d.property, " click"), function () {
        ...
        if (!active) {
          ...
          if (d.name === 'Listing') {
            ...
            context.containers.visitExpectationLegend.past.rect.classed('pvl-hidden', false);
            context.containers.visitExpectationLegend.future.rect.classed('pvl-hidden', false);
            ...
            context.containers.visitExpectationLegend.future.rect.classed('pvl-hidden', true);
            ...
            context.containers.visitExpectationLegendContainer.classed('pvl-hidden', false);
            context.containers.visitExpectationLegend.past.rect.classed('pvl-hidden', context.containers.ordinalChart.classed('pvl-hidden'));
            context.containers.visitExpectationLegend.future.rect.classed('pvl-hidden', context.containers.ordinalChart.classed('pvl-hidden'));
          }
        }
      });
    });
  ...
  function layout() {
    ...
    this.containers.lowerRow = this.containers.main.append('div').classed('pvl-row pvl-row--lower', true); //  tabs

    var selectedTabs = tabs.filter(function (tab) {
      return _this.settings.chart_layout === 'tabbed' ? tab.name !== 'Charts' : _this.settings.chart_layout === 'side-by-side' ? ['Visit Chart', 'Study Day Chart'].indexOf(tab.name) < 0 : true;
    });
    this.containers.tabContainer = this.containers.lowerRow.append('div').classed('pvl-tabs', true);
    this.containers.visitExpectationLegendContainer = this.containers.tabContainer.append('div').classed('pvl-visit-expectation-legend pvl-hidden', true);
    this.containers.nParticipants = this.containers.tabContainer.append('div').classed('pvl-viewing-n-participants', true).html('Viewing <span class = "pvl-n-participants"></span> participants.');
    this.containers.loading = this.containers.tabContainer.append('div').classed('pvl-hidden pvl-loading', true);
    this.containers.loading.selectAll('div.pvl-loading-ball').data([1, 2, 3]).enter().append('div').attr('class', function (d) {
      return "pvl-loading-ball pvl-loading-ball--".concat(d);
    });
    this.containers.tabs = this.containers.tabContainer.selectAll('div.pvl-tab').data(selectedTabs).enter().append('div').attr('class', function (d) {
      return "pvl-tab pvl-tab--".concat(d["class"], " ").concat(d.name === _this.settings.active_tab ? 'pvl-tab--active' : '');
    }).text(function (d) {
      return d.name;
    }); //  display containers

    if (this.settings.chart_layout === 'tabbed') {
      this.containers.ordinalChart = this.containers.lowerRow.append('div').classed('pvl-display pvl-chart pvl-chart--ordinal pvl-chart--full', true);
      this.containers.linearChart = this.containers.lowerRow.append('div').classed('pvl-display pvl-chart pvl-chart--linear pvl-chart--full', true);
    } else {
      this.containers.charts = this.containers.lowerRow.append('div').classed('pvl-charts', true);
      this.containers.ordinalChart = this.containers.charts.append('div').classed('pvl-display pvl-chart pvl-chart--ordinal pvl-chart--side-by-side', true);
      this.containers.linearChart = this.containers.charts.append('div').classed('pvl-display pvl-chart pvl-chart--linear pvl-chart--side-by-side', true);
    }

    this.containers.listing = this.containers.lowerRow.append('div').classed('pvl-display pvl-listing', true);
    /**-------------------------------------------------------------------------------------------\
      Functionality
    \-------------------------------------------------------------------------------------------**/
  ...
  function styles() {
    this.styles = ['html.pvl-wait, html.pvl-wait * {' + '    cursor: wait !important;' + '}', 'body.pvl-wait, body.pvl-wait * {' + '    cursor: wait !important;' + '}', '.participant-visit-listing {' + '    font-family: "Open Sans", "Helvetica Neue", Helvetica, Arial, sans-serif;' + '    font-size: 16px;' + '    line-height: normal;' + '}', '.pvl-hidden {' + '    display: none !important;' + '}', '.participant-visit-listing > * {' + '    width: 100%;' + '}',
    /***--------------------------------------------------------------------------------------\
      Upper row
    \--------------------------------------------------------------------------------------***/
    '.pvl-row--upper {' + '    padding-bottom: 12px;' + '    display: flex;' + '}', '.pvl-row--upper > div {' + '    flex: 1;' + '}',
    /****---------------------------------------------------------------------------------\
      Legend
    \---------------------------------------------------------------------------------****/
    '.pvl-legend {' + '    width: 35%;' + '    float: left;' + '    position: relative;' + '}', '.pvl-legend:after {' + '    content: "";' + '    display: inline-block;' + '    height: 100%;' + '    vertical-align: bottom;' + '}' + '.pvl-legend__label {' + '    font-size: 24px;' + '    font-weight: lighter;' + '    position: absolute;' + '    top: 0;' + '    left: 0;' + '}', '.pvl-legend__ul {' + '    list-style-type: none;' + '    margin: 0;' + '    padding: 0;' + '    overflow: hidden;' + '    display: inline-block;' + '    vertical-align: bottom;' + '    width: 100%;' + '}', '.pvl-legend__li {' + '    float: left;' + '    margin-right: 1%;' + '    text-align: center;' + '}', '.pvl-legend-item-info-icon {' + '    margin-left: 4px;' + '    font-weight: bold;' + '    cursor: help;' + '}',
    /****---------------------------------------------------------------------------------\
      Controls
    \---------------------------------------------------------------------------------****/
    '.pvl-controls {' + '    width: 64%;' + '    float: right;' + '    position: relative;' + '}', '.pvl-controls .pvl-reset-button {' + '    position: absolute;' + '    bottom: 0;' + '    left: 0;' + '}', '.pvl-controls .wc-controls {' + '    float: right;' + '    margin-bottom: 0;' + '    width: 100%;' + '}', '.pvl-controls .wc-controls .control-group {' + '    margin: 0 .8% 0 0;' + '    width: 16%;' + '}', '.pvl-controls .wc-controls .control-group:last-child {' + '    margin-right: 0;' + '}', '.pvl-controls .wc-controls .control-group:last-child {' + '    margin-right: 0;' + '}', '.pvl-controls .wc-controls .control-group > * {' + '    width: 100%;' + '}', '.pvl-controls .wc-controls .control-group .wc-control-label {' + '    margin-right: 5px;' + '    text-align: right;' + '    font-size: 14px;' + '}',
    /***--------------------------------------------------------------------------------------\
      Lower row
    \--------------------------------------------------------------------------------------***/
    ...
    /****---------------------------------------------------------------------------------\
      Tabs
    \---------------------------------------------------------------------------------****/
    '.pvl-tabs {' + '    text-align: center;' + '    border-top: 1px solid lightgray;' + '    border-bottom: 1px solid lightgray;' + '    padding: 6px 0;' + '    position: relative;' + '}', '.pvl-visit-expectation-legend {' + '    display: inline-block;' + '    position: absolute;' + '    top: 0;' + '    left: 0;' + '}', '.pvl-viewing-n-participants {' + '    display: inline-block;' + '    position: absolute;' + '    bottom: 0;' + '    right: 0;' + '}', '.pvl-n-participants {' + '    font-weight: bold;' + '}', '.pvl-tab {' + '    display: inline-block;' + '    border: 2px solid black;' + '    border-radius: 6px;' + '    padding: 1px 24px;' + '    font-size: 20px;' + '    margin: 0 2px;' + '    color: black;' + '    background: white;' + '    cursor: pointer;' + '    font-weight: normal;' + '}', '.pvl-tab--active {' + '    color: white;' + '    background: black;' + '    font-weight: bold;' + '    cursor: default;' + '}', '.pvl-tab:hover {' + '    color: white;' + '    background: black;' + '    font-weight: bold;' + '}', '.pvl-loading {' + '    width: 100px;' + '    display: inline-block;' + '    position: absolute;' + '    top: 75px;' + '    left: 50%;' + '    margin-left: -50px;' + '}', '.pvl-loading > div {' + '    width: 15px;' + '    height: 15px;' + '    background-color: #0458ad;' + '    border-radius: 100%;' + '    display: inline-block;' + '    -webkit-animation: sk-bouncedelay 1.4s infinite ease-in-out both;' + '    animation: sk-bouncedelay 1.4s infinite ease-in-out both;' + '}', '.pvl-loading .pvl-loading-ball--1 {' + '    -webkit-animation-delay: -0.32s;' + '    animation-delay: -0.32s;' + '}', '.pvl-loading .pvl-loading-ball--2 {' + '    -webkit-animation-delay: -0.16s;' + '    animation-delay: -0.16s;' + '}', '@-webkit-keyframes sk-bouncedelay {' + '    0%, 80%, 100% { -webkit-transform: scale(0) }' + '    40% { -webkit-transform: scale(1.0) }' + '}', '@keyframes sk-bouncedelay {' + '    0%, 80%, 100% { ' + '        -webkit-transform: scale(0);' + '        transform: scale(0);' + '    } 40% { ' + '        -webkit-transform: scale(1.0);' + '        transform: scale(1.0);' + '    }' + '}',
    /****---------------------------------------------------------------------------------\
      Charts
    \---------------------------------------------------------------------------------****/
    '.pvl-charts {' + '    width: 100%;' + '    display: inline-block;' + '}', '.pvl-chart {' + '    display: inline-block;' + '    width: 100%;' + '}', '.pvl-charts .pvl-chart--ordinal {' + '    width: 49.5%;' + '    float: left;' + '}', '.pvl-charts .pvl-chart--linear {' + '    width: 49.5%;' + '    float: right;' + '}', '.pvl-floating-axis {' + '    width: 100%;' + '    z-index: 99;' + '    position: static;' + '}', '.pvl-floating-axis.pvl-sticky {' + '    position: fixed;' + '    top: 0;' + '    background-color: #F6F6F6;' + '    opacity: 0.8;' + '}', '.pvl-chart .axis-title--top {' + '    font-size: 16px;' + '    font-weight: bold;' + '}' + '.pvl-chart--full .pvl-chart-button {' + '    display: none;' + '}', '.pvl-chart .pvl-chart-button {' + '    font-size: 30px;' + '    cursor: pointer;' + '    fill: black;' + '}' + '.pvl-chart .pvl-chart-button:hover {' + '    fill: blue;' + '    stroke: blue;' + '}' + '.pvl-chart .pvl-chart-button--minimize {' + '}' + '.pvl-chart .pvl-chart-button--split {' + '    font-size: 24px;' + '}' + '.pvl-chart .pvl-chart-button--split:hover {' + '}' + '.pvl-chart .pvl-chart-button--maximize {' + '}' + '.pvl-chart--ordinal .x.axis .tick {' + '    cursor: help;' + '}', '.pvl-unscheduled-legend-item,' + '.pvl-unscheduled-annotation {' + '    font-size: 14px;' + '    font-family: courier;' + '}', '.pvl-chart--linear .wc-data-mark {' + '    cursor: pointer;' + '    clip-path: none !important;' + '}', '.pvl-chart--linear .pvl-highlighted-visit-mark {' + '    cursor: pointer;' + '}',
    /****---------------------------------------------------------------------------------\
      Listing
    \---------------------------------------------------------------------------------****/
    '.pvl-listing {' + '}', '.pvl-listing .pvl-scroll-bar--outer {' + '    width: 100%;' + '    overflow-x: auto;' + '    overflow-y: hidden;' + '}', '.pvl-listing .pvl-scroll-bar--upper {' + '    height: 20px;' + '}', '.pvl-listing .wc-table {' + '    overflow: none;' + '}', '.interactivity.pvl-cell-text-toggle {' + '    margin-right: 10px;' + '    border: 1px solid #aaa;' + '    border-radius: 5px;' + '    padding: 5px;' + '}', '.pvl-cell-text-toggle__label {' + '}', '.pvl-cell-text-toggle__checkbox {' + '    margin-left: 5px;' + '}', '.pvl-listing .wc-table table {' + '    display: table;' + '    border: 0;' + '    border-collapse: collapse;' + '    min-width: 100%;' + '}',
    /*****----------------------------------------------------------------------------\
      thead
    \----------------------------------------------------------------------------*****/
  ...
  function defineColumns() {
    this.config.cols = this.display_dates ? ['Site', 'ID', 'Status'].concat(this.pvl.data.sets.scheduledVisits.map(function (visit) {
  ...
  function addTopScrollBar() {
    this.scrollBars = {};
    this.scrollBars.upper_outer = this.pvl.containers.listing.insert('div', ':first-child').classed('pvl-scroll-bar pvl-scroll-bar--upper pvl-scroll-bar--outer', true);
    this.scrollBars.upper_inner = this.scrollBars.upper_outer.append('div').classed('pvl-scroll-bar pvl-scroll-bar--upper pvl-scroll-bar--inner', true);
    this.scrollBars.lower_outer = this.pvl.containers.listing.insert('div', '.wc-chart').classed('pvl-scroll-bar pvl-scroll-bar--lower pvl-scroll-bar--outer', true);
    this.scrollBars.lower_inner = this.wrap.classed('pvl-scroll-bar pvl-scroll-bar--lower pvl-scroll-bar--inner', true);
  ...
  function clearSort() {
    ...
    if (this.sortable.order.some(function (item) {
  ...
  function visit() {
    ...
    this.pvl.data.sets.visit_col.forEach(function (visit) {
      ...
      visit_cell.attr('title', "".concat(visit.name, "\n - ").concat(visit_summary.map(function (status) {
        return "".concat(status.key, " (").concat(status.values, ")");
      }).join('\n - ')));
      if (_this.pvl.settings.abbreviate_visits) visit_cell.text(function (d) {
        var abbreviation = _this.pvl.data.sets.visit_col.find(function (visit) {
          return visit.name === d;
        }).abbreviation;

        return abbreviation !== 'undefined' ? _this.pvl.data.sets.visit_col.find(function (visit) {
          return visit.name === d;
        }).abbreviation : d;
      });
    });
  ...
  function exportXLSX() {
    if (!this.pvl.test) {
      var fileName = "participant-visit-listing-".concat(d3.time.format('%Y-%m-%dT%H-%M-%S')(new Date()), ".xlsx");
  ...
  function exportToXLSX() {
    ...
    if (this.config.exportable) this.wrap.select('.export#xlsx').on('click', function () {
  ...
  function exportToPDF() {
    ...
    if (this.config.exportable) this.wrap.select('.export#pdf').on('click', function () {
  ...
  function exportToCSV() {
    ...
    if (this.config.exportable) this.wrap.select('.export#csv').on('click', function () {
  ...
  function onDraw() {
    ...
    if (this.pvl.settings.active_tab === 'Listing') this.pvl.containers.loading.classed('pvl-hidden', true);
  }
  ...
  function minimize() {
    ...
    loading.call(this.pvl, "".concat(this.property, ".minimize()"), function () {
      ...
      _this.pvl.containers.visitExpectationLegend.past.rect.classed('pvl-hidden', thatChart === 'linearChart');

      _this.pvl.containers.visitExpectationLegend.future.rect.classed('pvl-hidden', thatChart === 'linearChart');
    });
  }
  ...
  function maximize() {
    ...
    loading.call(this.pvl, "".concat(this.property, ".maximize()"), function () {
      ...
      _this.pvl.containers.visitExpectationLegend.past.rect.classed('pvl-hidden', thisChart === 'linearChart');

      _this.pvl.containers.visitExpectationLegend.future.rect.classed('pvl-hidden', thisChart === 'linearChart');
    });
  }
  ...
  function addButtons() {
    ...
    this.topXAxis.minimize = this.topXAxis.svg.append('text').classed('pvl-chart-button pvl-chart-button--minimize', true).text("\u2212").on('click', function () {
      return minimize.call(_this);
    });
    this.topXAxis.minimize.append('title').text('MinimizeChart'); //  Add split chart button.

    this.topXAxis.split = this.topXAxis.svg.append('text').classed('pvl-chart-button pvl-chart-button--split', true).text("\u25A1\u25A1").on('click', function () {
      return split.call(_this);
    });
    this.topXAxis.split.append('title').text('View both charts'); //  Add maximize chart button.

    this.topXAxis.maximize = this.topXAxis.svg.append('text').classed('pvl-chart-button pvl-chart-button--maximize', true).text("+").on('click', function () {
  ...
  function scrollTopXAxis() {
    ...
    window.addEventListener('scroll', function () {
      ...
      if (window_top > div_top) {
        if (!context.topXAxis.container.classed('pvl-sticky')) context.topXAxis.container.classed('pvl-sticky', true);
      } else {
  ...
  function addTooltipsToXAxisTicks() {
    ...
    this.wrap.selectAll('.x.axis .tick').append('title').classed('pvl-x-tooltip', true).text(function (d) {
  ...
  function mousemove(mouse) {
    ...
    if (this.config.x.type === 'ordinal') {
      ...
      if (x > 0) this.topXAxis.svg.append('g').classed('tick tick--highlight', true).append('text').classed('pvl-highlight-x-tick-label', true).attr({
        x: x,
        y: -9,
        'text-anchor': 'middle'
      }).style({
  ...
  function abbreviateXAxisTickLabels() {
    ...
    if (this.pvl.settings.abbreviate_visits) this.wrap.selectAll('.x.axis .tick text').text(function (d) {
      var abbreviation = _this.pvl.data.sets.visit_col.find(function (visit) {
        return visit.name === d;
      }).abbreviation;
  ...
  function updateTextMarks() {
    this.annotations.selectAll('text').classed('pvl-unscheduled-annotation', true).style('clip-path', null);
  }
  ...
  function clearHighlight() {
    var deleteHighlight = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : true;
    ...
    if (this.highlight) {
      ...
      this.points.classed('pvl-highlighted-visit', false).selectAll('circle').attr({
        'fill-opacity': 1,
        'stroke-opacity': 1
  ...
  function highlightVisit() {
    ...
    this.marks.forEach(function (mark) {
      mark.groups.on('mouseover', function (d) {
        mouseover.call(context, this, d);
      }).on('click', function (d) {
        if (!context.highlight || context.highlight.visit !== d.values.raw[0][context.pvl.settings.visit_col]) click.call(context, this, d);else clearHighlight.call(context);
  ...
  function checkFilterCol(input) {
    ...
    if (!this.data.missingVariables[filterCol]) {
      ...
      this.ordinalChart.controls.config.inputs = this.ordinalChart.controls.config.inputs.filter(function (input) {
        return input.value_col !== filterCol;
      });
      this.linearChart.controls.config.inputs = this.linearChart.controls.config.inputs.filter(function (input) {
        return input.value_col !== filterCol;
      });
      this.listing.controls.config.inputs = this.listing.controls.config.inputs.filter(function (input) {
        return input.value_col !== filterCol;
      });
    } else {
  ...
  function checkRequiredVariables() {
    ...
    this.settings.controlsSynced.inputs.filter(function (input) {
  ...
  function recordLevel() {
    ...
    this.data.raw.forEach(function (d) {
      ...
      d.expected = _this.settings.visit_expectation_regex ? _this.settings.visit_expectation_regex.test(d[_this.settings.visit_status_col]) : false;
      d.unscheduled = _this.settings.visit_exclusion_regex ? _this.settings.visit_exclusion_regex.test(d[_this.settings.visit_col]) : false;
      d.overdue = _this.settings.visit_overdue_regex ? _this.settings.visit_overdue_regex.test(d[_this.settings.visit_status_col]) : false;
    });
  }
  ...
  function defineLegendSet() {
    ...
    this.data.sets.past_visits = this.data.sets.legend.map(function (visit_status) {
      return visit_status.split(':|:')[1];
    }).filter(function (visit_status) {
      return !_this.settings.visit_expectation_regex.test(visit_status);
    });
    this.data.sets.future_visits = this.data.sets.legend.map(function (visit_status) {
      return visit_status.split(':|:')[1];
    }).filter(function (visit_status) {
      return _this.settings.visit_expectation_regex.test(visit_status);
    });
  ...
  function calculateVisitStatistics() {
    ...
    this.data.statistics.visits = this.data.sets.visit_col.map(function (visit) {
      return visit.name;
    ...
    Object.keys(this.data.statistics.visits).map(function (visit) {
      return _this.data.statistics.visits[visit];
    }).forEach(function (visit) {
      visit.data = _this.data.raw.filter(function (d) {
        return d[_this.settings.visit_col] === visit.name;
      });
      visit.days = visit.data.map(function (d) {
        return +d[_this.settings.visit_day_col];
      }).sort(function (a, b) {
        return a - b;
      });
      visit.n = visit.days.length;
      statistics.filter(function (statistic) {
        return statistic !== 'n';
      }).forEach(function (statistic) {
        visit[statistic] = d3[statistic](visit.days);
      });
      visit.tooltip = [visit.name, "Reference day: ".concat(visit.median, " (median)"), "Statistics:"].concat(_toConsumableArray(statistics.map(function (statistic) {
        return " - ".concat(statistic, ": ").concat(['mean', 'deviation'].includes(statistic) ? d3.format('.1f')(visit[statistic]) : Math.round(visit[statistic]));
  ...
  function addVisitStatusStyles() {
    var visitStatusStyles = this.data.sets.visit_status_col.map(function (visit_status) {
      var split = visit_status.split(':|:');
      var order = split[0];
      var status = split[1].toLowerCase().replace(/[^_a-z-]/g, '-'); //  .replace(/ /g, '.');

      var color = split[2];
      var styles = [".pvl-visit-status--".concat(status, " {"), "    border-top: 2px solid ".concat(color, ";"), "    border-bottom: 2px solid ".concat(color, ";"), "}", ".pvl-visit-status--heat-map.pvl-visit-status--".concat(status, " {"), "    background: ".concat(color, ";"), "    color: transparent;", "    opacity: .9;", "}", ".pvl-visit-status--cell-text.pvl-visit-status--".concat(status, " {"), "    color: ".concat(color, ";"), "    opacity: 1;", "}", "tr:nth-child(odd) .pvl-visit-status--cell-text.pvl-visit-status--".concat(status, " {"), "    background: white;", "}", "tr:nth-child(even) .pvl-visit-status--cell-text.pvl-visit-status--".concat(status, " {"), "    background: #eee;", "}"];
      return styles.join('\n');
    }).join('\n');
    this.containers.style.html("".concat(this.containers.style.html(), "\n").concat(visitStatusStyles));
  }
  ...
  function addVisitStatusLegend() {
    ...
    this.containers.legendItems.each(function (d) {
      ...
      legendItem.append('span').classed('pvl-legend-item-info-icon', true).html('&#9432').style({
        ...
        return context.settings.visit_expectation_regex && context.settings.visit_expectation_regex.test(d[1]) ? "".concat(d[3], "\n").concat(d[1], " visits are identified in the charts as cells or circles with medial white circles.") : d[3];
      });
    });
    update.call(this);
  }
  ...
  function addVisitExpectationLegend() {
    ...
    this.containers.visitExpectationLegend = {
      g: this.containers.visitExpectationLegendContainer.append('svg').attr({
        width: 400,
        height: this.containers.tabContainer.node().offsetHeight
      }).append('g').classed('pvl-mark-legend', true)
    }; //  past visits

    this.containers.visitExpectationLegend.past = {
      g: this.containers.visitExpectationLegend.g.append('g').classed('pvl-mark-legend__item pvl-mark-legend__item--past', true).attr({
        transform: "translate(0,".concat(4, ")")
      })
    };
    this.containers.visitExpectationLegend.past.rect = this.containers.visitExpectationLegend.past.g.append('rect').classed('pvl-legend-rect pvl-legend-rect--past', true).attr(rectAttributes.call(this));
    this.containers.visitExpectationLegend.past.outerCircle = this.containers.visitExpectationLegend.past.g.append('circle').classed('pvl-legend-circle pvl-legend-circle--outer pvl-legend-circle--past', true).attr(outerCircleAttributes.call(this));
    this.containers.visitExpectationLegend.past.text = this.containers.visitExpectationLegend.past.g.append('text').classed('pvl-legend-text pvl-legend-text--past', true).attr(textAttributes.call(this).attr).style(textAttributes.call(this).style);
    this.containers.visitExpectationLegend.past.text.selectAll('tspan').data(this.data.sets.past_visits).enter().append('tspan').classed('pvl-legend-tspan pvl-legend-tspan--past', true).attr('fill', function (d) {
      return _this.data.sets.visit_status_col.find(function (visit_status) {
    ...
    this.containers.visitExpectationLegend.future = {
      g: this.containers.visitExpectationLegend.g.append('g').classed('pvl-mark-legend__item pvl-mark-legend__item--future', true).attr({
        transform: "translate(0,".concat(4 * 2 + this.settings.ordinalChartSynced.y.range_band, ")")
      })
    };
    this.containers.visitExpectationLegend.future.rect = this.containers.visitExpectationLegend.future.g.append('rect').classed('pvl-legend-rect pvl-legend-rect--future', true).attr(rectAttributes.call(this));
    this.containers.visitExpectationLegend.future.outerCircle = this.containers.visitExpectationLegend.future.g.append('circle').classed('pvl-legend-circle pvl-legend-circle--outer pvl-legend-circle--future', true).attr(outerCircleAttributes.call(this));
    this.containers.visitExpectationLegend.future.innerCircle = this.containers.visitExpectationLegend.future.g.append('circle').classed('pvl-legend-circle pvl-legend-circle--inner pvl-legend-circle--future', true).attr(innerCircleAttributes.call(this));
    this.containers.visitExpectationLegend.future.text = this.containers.visitExpectationLegend.future.g.append('text').classed('pvl-legend-text pvl-legend-text--future', true).attr(textAttributes.call(this).attr).style(textAttributes.call(this).style);
    this.containers.visitExpectationLegend.future.text.selectAll('tspan').data(this.data.sets.future_visits).enter().append('tspan').classed('pvl-legend-tspan pvl-legend-tspan--future', true).attr('fill', function (d) {
      return _this.data.sets.visit_status_col.find(function (visit_status) {
        return visit_status.split(':|:')[1] === d;
      }).split(':|:')[2];
    }).text(function (d, i) {
    ...
    this.containers.visitExpectationLegend.past.rect = this.containers.visitExpectationLegendContainer.select('rect.pvl-legend-rect--past');
    this.containers.visitExpectationLegend.future.rect = this.containers.visitExpectationLegendContainer.select('rect.pvl-legend-rect--future');
  }
  ...
  function updateMultiSelects$1() {
    this.controls.wrap.selectAll('.control-group').filter(function (d) {
      return d.multiple;
    }).selectAll('select').selectAll('option').property('selected', true);
  }
