...
function styles() {
            this.styles = ['body {' + '}', '.participant-visit-listing {' + '    font-family: "Open Sans", "Helvetica Neue", Helvetica, Arial, sans-serif;' + '    font-size: 16px;' + '    line-height: normal;' + '}', '.participant-visit-listing > * {' + '    width: 100%;' + '    display: inline-block;' + '}', '.pvl-row--upper {' + '    border-bottom: 2px solid #eee;' + '    padding-bottom: 12px;' + '}', '.pvl-row--upper > * {' + '    width: 45%;' + '    vertical-align: bottom;' + '}', '.pvl-controls {' + '    display: inline-block;' + '    width: 45%;' + '    float: right;' + '}', '.pvl-controls .wc-controls {' + '    float: right;' + '    margin-bottom: 0;' + '}', '.pvl-controls .wc-controls .control-group {' + '    margin-bottom: 0;' + '}', '.pvl-controls .wc-controls .control-group > * {' + '    display: inline-block;' + '}', '.pvl-controls .wc-controls .control-group .wc-control-label {' + '    margin-right: 5px;' + '}', '.pvl-legend {' + '    width: 45%;' + '    float: left;' + '}', '.pvl-legend__ul {' + '    list-style-type: none;' + '    margin: 0;' + '    padding: 0;' + '    overflow: hidden;' + '}', '.pvl-legend__li {' + '    float: left;' + '    margin-right: 10px;' + '    width: 150px;' + '    text-align: center;' + '}', '.pvl-listing {' + '}', '.pvl-listing .wc-table {' + '    width: 100%;' + '    overflow-x: scroll;' + '}',
...
function participant() {
    ...
    this.parent.data.sets.id_col.forEach(function (id) {
        ...
        var id_cell = _this.table.selectAll('tbody tr').selectAll('td:nth-child(2)').filter(function (d) {
            return d.text.indexOf(id) > -1;
        }) // define a more rigid selector here
        .filter(function (d, i) {
            return i === 0;
        });
        id_cell.attr('title', id_summary.map(function (status) {
