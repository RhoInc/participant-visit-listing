(function (global, factory) {
  ...
  !function (global, factory) {
      ...
      defineProperties(_Symbol, {
          ...
          writable: !0,
          configurable: !0
      }), simpleInheritance(ArrayIterator, Iterator), simpleInheritance(StringIterator, Iterator), defineProperty$$1(ArrayIterator.prototype, _Symbol.toStringTag.toString(), {
          ...
          var nextValue;
          return -1 === this._nextIndex ? { done: !0, value: void 0 } : 'number' == typeof this._array.length && this._array.length >= 0 && this._nextIndex < Math.floor(this._array.length) ? (1 === this._flag ? nextValue = [this._nextIndex, this._array[this._nextIndex]] : 2 === this._flag ? nextValue = this._array[this._nextIndex] : 3 === this._flag && (nextValue = this._nextIndex), this._nextIndex++, { done: !1, value: nextValue }) : (this._nextIndex = -1, { done: !0, value: void 0 });
          ...
          return -1 === this._nextIndex ? { done: !0, value: void 0 } : this._nextIndex < stringObject.length ? (nextValue = stringObject[this._nextIndex], this._nextIndex++, { done: !1, value: nextValue }) : (this._nextIndex = -1, { done: !0, value: void 0 });
      };
      ...
      return defineProperties(ES6, {
          ...
          instanceOf: {
              value: function value(object, constructor) {
                  ...
                  if ('function' != typeof hasInstanceSymbolProp) throw new TypeError((typeof hasInstanceSymbolProp === 'undefined' ? 'undefined' : _typeof(hasInstanceSymbolProp)) + ' is not a function');
      ...
      }), defineProperty$$1(global, 'Symbol', { value: _Symbol, writable: !0, configurable: !0 }), defineProperty$$1(Function.prototype, _Symbol.hasInstance.toString(), {
  ...
  function syncListingSettings() {
      ...
      if (['tabbed', 'side-by-side'].indexOf(settings.chart_layout) < 0) {
          console.warn('[ chart_layout ] must be "tabbed" or "side-by-side", not "' + settings.chart_layout + '". Defaulting to "tabbed".');
      ...
      if (settings.chart_layout === 'tabbed') {
          if (['Visit Chart', 'Study Day Chart', 'Listing'].indexOf(settings.active_tab) < 0) {
              console.warn('[ active_tab ] must be "Visit Chart", "Study Day Chart", or "Listing", not "' + settings.active_tab + '". Defaulting to "Visit Chart".');
              settings.active_tab = 'Visit Chart';
          }
      } else if (settings.chart_layout === 'side-by-side') {
          if (['Charts', 'Listing'].indexOf(settings.active_tab) < 0) {
              console.warn('[ active_tab ] must be "Charts" or "Listing", not "' + settings.active_tab + '". Defaulting to "Charts".');
  ...
  function transposeData() {
      ...
      var _loop = function _loop(id) {
          ...
          var datum = (_datum = {}, defineProperty(_datum, _this.settings.site_col, id_data[0][_this.settings.site_col]), defineProperty(_datum, 'Site', id_data[0][_this.settings.site_col]), defineProperty(_datum, _this.settings.id_col, id), defineProperty(_datum, 'ID', id), defineProperty(_datum, _this.settings.id_status_col, id_data[0][_this.settings.id_status_col]), defineProperty(_datum, 'Status', id_data[0][_this.settings.id_status_col]), _datum);
  ...
  function layout() {
      ...
      this.containers.loading = this.containers.tabContainer.append('div.pvl-loading').classed('pvl-hidden pvl-loading', true);
      ...
      this.containers.tabs = this.containers.tabContainer.selectAll('div.pvl-tab').data(selectedTabs).enter().append('div').attr('class', function (d) {
          return 'pvl-tab pvl-tab--' + d.class + ' ' + (d.name === _this.settings.active_tab ? 'pvl-tab--active' : '');
      }).text(function (d) {
          return d.name;
      });
  ...
  function addCellFormatting() {
      ...
      this.tbody.selectAll('tr').each(function (d) {
          var visitCells = d3.select(this).selectAll('td:nth-child(n + 4)').attr('class', function (di) {
              return d[di.col + '-status'] ? 'pvl-visit-status--' + d[di.col + '-status'].toLowerCase().replace(/[^_a-z-]/g, '-') : '';
          ...
          visitCells.each(function (di) {
              ...
              if (d[di.col] !== null) visitCell.attr('title', d[context.pvl.settings.id_col] + ' - ' + di.col + ' (' + di.date + '): ' + d[di.col + '-status']);
          });
      });
  }
  ...
  function addLegend() {
      ...
      this.containers.legendItems.each(function (d) {
          ...
          legendItem.append('span').classed('pvl-legend-item-info-icon', true).html('&#9432').style({
              ...
              return context.settings.visit_expectation_regex && context.settings.visit_expectation_regex.test(d[1]) ? d[3] + '\n' + d[1] + ' visits are identified in the charts as cells or circles with medial white circles.' : d[3];
          });
      });
      update.call(this);
  }
