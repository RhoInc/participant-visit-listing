(function (global, factory) {
    ...
    function styles() {
      ...
      '.pvl-legend {' + '    width: 44%;' + '    float: left;' + '    text-align: center;' + '}', '.pvl-legend__label {' + '    font-size: 24px;' + '    font-weight: lighter;' + '}', '.pvl-legend__ul {' + '    list-style-type: none;' + '    margin: 0;' + '    padding: 0;' + '    overflow: hidden;' + '}', '.pvl-legend__li {' + '    float: left;' + '    margin-right: 1%;' + '    width: 24%;' + '    text-align: center;' + '}', '.pvl-legend-item-info-icon {' + '    margin-left: 4px;' + '    font-weight: bold;' + '    cursor: help;' + '}',

      /****---------------------------------------------------------------------------------\
        Controls
      \---------------------------------------------------------------------------------****/

      '.pvl-controls {' + '    width: 55%;' + '    float: right;' + '}', '.pvl-controls .wc-controls {' + '    float: right;' + '    margin-bottom: 0;' + '}', '.pvl-controls .wc-controls .control-group {' + '    margin-bottom: 0;' + '    width: 125px;' + '}', '.pvl-controls .wc-controls .control-group:last-child {' + '    margin-right: 0;' + '}', '.pvl-controls .wc-controls .control-group > * {' + '    width: 100%;' + '}', '.pvl-controls .wc-controls .control-group .wc-control-label {' + '    margin-right: 5px;' + '    text-align: right;' + '}',
      ...
      '.pvl-charts {' + '    width: 100%;' + '    display: inline-block;' + '}', '.pvl-chart {' + '    display: inline-block;' + '}', '.pvl-chart--ordinal {' + '    width: 49.5%;' + '    float: left;' + '}', '.pvl-chart--linear {' + '    width: 49.5%;' + '    float: right;' + '}', '.pvl-chart .axis-title--top {' + '    font-size: 16px;' + '    font-weight: bold;' + '}' + '.pvl-chart .pvl-chart-button {' + '    font-size: 24px;' + '    cursor: pointer;' + '}' + '.pvl-chart .pvl-chart-button:hover {' + '    font-weight: bold;' + '}' + '.pvl-chart .pvl-chart-button--minimize {' + '}' + '.pvl-chart .pvl-chart-button--split {' + '    font-size: 12px;' + '}' + '.pvl-chart .pvl-chart-button--maximize {' + '}' +
        ...
        this.controls.wrap.on('change', function () {
            ...
            context.listing.filters.forEach(function (filter) {
                context.data.filtered = context.data.filtered.filter(function (d) {
                    return Array.isArray(filter.val) ? filter.val.indexOf(d[filter.col]) > -1 : filter.val === 'All' || d[filter.col] === filter.val;
                });
            });
            update.call(context);
        });
    ...
    function addButtons() {
        ...
        this.topXAxis.minimize = this.topXAxis.container.append('text').classed('pvl-chart-button pvl-chart-button--minimize', true).html('&minus;<title>Minimize chart</title').attr('title', 'Minimize chart').on('click', function () {
            return minimize.call(_this);
        });

        //Add split chart button.
        this.topXAxis.split = this.topXAxis.container.append('text').classed('pvl-chart-button pvl-chart-button--split', true).html('&#8418;&#8418;<title>View both charts</title>').attr('title', 'View both chart').on('click', function () {
            return split.call(_this);
        });

        //Add maximize chart button.
        this.topXAxis.maximize = this.topXAxis.container.append('text').classed('pvl-chart-button pvl-chart-button--maximize', true).html('&plus;<title>Maximize chart').attr('title', 'Maximize chart').on('click', function () {
    ...
    function checkFilterCols(filterCol) {
        ...
        if (!this.data.missingVariables[filterCol]) this.settings.controlsSynced.inputs = this.settings.controlsSynced.inputs.filter(function (input) {
            ...
            this.settings.controlsSynced.inputs.find(function (input) {
                return input.value_col === filterCol;
            }).values = d3.set(this.data.raw.map(function (d) {
                return d[filterCol];
    ...
    function defineVisitSet() {
        ...
        this.data.sets.visit_col = d3.set(this.data.filtered.map(function (d) {
    ...
    function transposeData() {
        ...
        this.data.sets.id_col.forEach(function (id, i) {
            ...
            _this.data.sets.visit_col.forEach(function (visit) {
                ...
                datum[visit + '-color'] = visit_datum ? visit_datum[_this.settings.visit_text_color_col] : '';
    ...
    function updateMultiSelects() {
        this.controls.wrap.selectAll('.control-group').filter(function (d) {
            return d.multiple;
        }).select('select').attr('size', 2).selectAll('option').property('selected', true);
    }
    ...
    function update$1() {
        ...
        var analysisSubsetters = this.controls.wrap.selectAll('.control-group').filter(function (d) {
            return (/^Analysis Subset \d$/.test(d.label)
            );
        }).selectAll('select');
        analysisSubsetters.selectAll('option').filter(function (d) {
            return d === 'All';
        }).filter(function (d, i) {
            return i > 0;
        }).remove();
